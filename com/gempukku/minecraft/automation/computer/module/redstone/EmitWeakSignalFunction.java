...
public class EmitWeakSignalFunction implements ModuleFunctionExecutable {
    @Override
    ...
    public Object executeFunction(int line, World world, ModuleComputerCallback computer, Map<String, Variable> parameters) throws ExecutionException {
        ...
        int side = StorageModuleUtils.getComputerFacingSide(line, computer, sideParam, "emitWeakSignal");

        if (strengthParam.getType() != Variable.Type.NUMBER)
            throw new ExecutionException(line, "Expected NUMBER in emitWeakSignal");
